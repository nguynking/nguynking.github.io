...
#
module Jekyll
    class RenderMarginFigureRadiusTag < Liquid::Tag
      ...
      def render(context)
        ...
        if @text[1].start_with?('http://', 'https://', '//')
          ...
          "<span class='marginnote'><img class='fullwidth' src='#{@text[1]}' style='border-radius:5px'/><br>#{@text[2]}</span>"
        else
          "<label for='#{@text[0]}' class='margin-toggle'>&#8853;</label>"+
          "<input type='checkbox' id='#{@text[0]}' class='margin-toggle'/>"+
  ...
  Liquid::Template.register_tag('marginfigureradius', Jekyll::RenderMarginFigureRadiusTag)
